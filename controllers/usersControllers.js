...
export function postUser (req, res) {
  ...
  if (!firstName || !lastName) 
    return res.status(400).json({ msg: 'Please provide both first name and last name' })
  ...
  if (!isAlphanumeric(firstName) || !isAlphanumeric(lastName))
    return res.status(400).json({ msg: 'First name and last name must be alphanumeric' })
